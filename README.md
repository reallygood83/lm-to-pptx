...
## 🎨 "검은 화면은 무서워요" (데스크탑 앱처럼 쓰기)

터미널이 어렵다면, 예쁜 화면을 띄워서 마우스로 클릭하며 쓸 수 있습니다!

```bash
nb2pptx --ui
```
👉 웹 브라우저가 열리면서 **앱 실행 화면**이 나옵니다. 파일을 드래그해서 넣기만 하면 끝!

---

## 💻 윈도우(Windows) 사용자 꿀팁

맥 사용자는 `brew install poppler` 한 방이면 되지만, 윈도우는 조금 더 손이 갑니다.
하지만 걱정 마세요!

1.  [Poppler 다운로드 (클릭)](https://github.com/oschwartz10612/poppler-windows/releases/)에서 최신 버전을 받습니다.
2.  압축을 풀고, `bin` 폴더 경로를 복사합니다.
3.  시스템 환경 변수 `Path`에 그 경로를 추가해주세요.
    *   *혹시 어렵다면 유튜브에 "윈도우 poppler 설치"라고 검색해보세요!*
